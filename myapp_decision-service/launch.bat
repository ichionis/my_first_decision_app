...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "myapp_decision-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name myapp_decision-service apps/myapp_decision-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app myapp_decision-service:1.0-SNAPSHOT
		call oc expose svc/myapp_decision-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd myapp_decision-service
		cd target
