...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "myapp_decision-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name myapp_decision-service apps/myapp_decision-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app myapp_decision-service:1.0-SNAPSHOT
    oc expose svc/myapp_decision-service
else

	echo "Launching the application locally..."
	pattern="myapp_decision-service"
	files=( $pattern )
	cd ${files[0]}
